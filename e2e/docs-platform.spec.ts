...
test("task guide has complete navigation and no accessibility violations", async ({
  ...
  await expect(
    page.getByRole("heading", { level: 1, name: "Run Tixkit locally" }),
  ).toBeVisible();
  await expect(
...
test("local search supports keyboard use and audience filtering", async ({
  page,
}) => {
  await page.goto("/");
  ...
  await expect(
    page.getByRole("link", { name: /Verify webhook signatures/ }).first(),
  ).toBeVisible();
  await input.fill("android troubleshoot");
  await expect(
    page.getByRole("link", { name: /Android SDK/ }).first(),
  ).toHaveAttribute("href", "/sdks/android#troubleshoot");
...
test("search failure is announced and retry recovers without page errors", async ({
  page,
}) => {
  let attempts = 0;
  ...
  await expect(
    page
      .getByRole("dialog", { name: "Search Tixkit documentation" })
      .getByRole("alert"),
...
test("framework tabs support arrow, Home, and End keyboard navigation", async ({
  page,
}) => {
...
for (const [label, route] of [
  ...
  test(`${label} SDK guide completes the first-request path`, async ({
    page,
  }, testInfo) => {
    await page.goto(route);
    await expect(
      page.getByRole("heading", { level: 2, name: "Install" }),
    ).toBeVisible();
    await expect(
      page.getByRole("heading", { level: 2, name: "Configure securely" }),
    ).toBeVisible();
    await expect(
      page.getByRole("heading", { level: 2, name: "Make the first request" }),
    ).toBeVisible();
    await expect(
      page.getByRole("heading", { level: 2, name: "Troubleshoot" }),
    ).toBeVisible();
    await expect(
...
test("mobile navigation remains operable without horizontal page overflow", async ({
  ...
  await expect(
    menu.getByRole("navigation", { name: "Mobile documentation" }),
  ).toBeVisible();
  const current = menu.getByRole("link", { name: "Events", exact: true });
  await expect(current).toHaveAttribute("aria-current", "page");
  await menu
    .getByRole("link", { name: "Create the first event", exact: true })
    .click();
  await expect(
...
for (const journey of [
  {
    ...
    stages: [
      ...
      [
        ...
        "Create and publish the first event",
        "Create the event",
      ],
      [
  ...
  for (const [stageIndex, [route, heading, task]] of journey.stages.entries()) {
    test(`${journey.persona} exposes canonical stage ${stageIndex + 1}`, async ({
      ...
      await expect(
        page.locator(".prose").getByText(task, { exact: false }).first(),
      ).toBeVisible();
      await expect(
      ...
      expect(
        results.violations,
        `${journey.persona} stage ${stageIndex + 1} violations`,
      ).toEqual([]);
    });
  }
}

test("Operator follows the event-to-checkout-to-check-in journey", async ({
  page,
}) => {
  ...
  await page
    ...
    .click();
  await expect(
    page.getByRole("heading", { level: 1, name: "Run check-in" }),
  ).toBeVisible();
});
...
test("Self-hoster follows configuration through deployment and observability", async ({
  page,
}) => {
  await page.goto("/self-hosting/configuration");
  await page.getByRole("link", { name: "deployment runbook" }).click();
  await expect(
    page.getByRole("heading", { level: 1, name: "Deploy Tixkit" }),
  ).toBeVisible();
...
test("Contributor follows setup through authoring and validation", async ({
  page,
}) => {
  await page.goto("/contributing/repository-setup");
  await page.getByRole("link", { name: "adding documentation" }).click();
  await expect(
    page.getByRole("heading", { level: 1, name: "Add documentation" }),
  ).toBeVisible();
