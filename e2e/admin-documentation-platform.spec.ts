...
test.describe('dashboard documentation platform', () => {
  ...
  test('Help Center searches permitted tasks and exact troubleshooting destinations', async ({
    ...
    await page.getByRole('searchbox', { name: 'Search Help Center' }).fill('signature');
    ...
    expect(
      destinations.some((href) => href.includes('/developers/webhooks/verify-signatures')),
    ).toBe(true);
  ...
  test('developer console keeps secrets out of the page and separates API and webhook guidance', async ({
    ...
    await expect(page.getByText('TIXKIT_API_KEY', { exact: false }).first()).toBeVisible();
    const body = await page.locator('body').innerText();
    expect(body).not.toMatch(/\btk_[A-Za-z0-9_-]{8,}/);
    const apiHref = await page.getByRole('link', { name: 'Open reference' }).getAttribute('href');
  ...
  test('readiness renders server-owned incomplete and complete states with explicit scope', async ({
    ...
    await page.route('**/organizations/*/readiness?*', async (route) => {
      ...
      const steps = [
        ...
        },
        {
          ...
          priority: 'required',
          reasonCodes: [complete ? 'brand_identity_configured' : 'brand_identity_incomplete'],
    ...
    expect(
      readinessRequests.every((url) => url.includes('/organizations/org_dev_local/readiness')),
    ).toBe(true);
    expect(readinessRequests.every((url) => url.includes('brandId=brd_dev_local'))).toBe(true);
  });
});
