...
Each directory is a minimal, buildable framework application using the public `@tixkit/checkout-headless@1.0.0` package contract. `scripts/test-checkout-headless-consumers.mjs` packs the local package, copies every example outside the monorepo, replaces only that dependency with the tarball, performs a clean install, and runs the framework's real production build.
