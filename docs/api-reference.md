# GateKit API Reference

GateKit exposes a JSON HTTP API at `API_BASE_URL` (default `http://localhost:4000` in development). All public/admin endpoints live under the `/v1` prefix. The API is built with Fastify and serves checkout, admin dashboard, scanner, and SDK traffic.
...
- **OpenAPI schema:** Generated by `@gatekit/openapi`. See `packages/openapi/src/index.ts` for the canonical spec.

## Authentication

GateKit supports three authenticated principal types plus anonymous buyer routes. The auth middleware picks the mode based on the request shape:
...
| API key | `Authorization: Bearer gk_<key>` | `api_key` | All authenticated routes allowed by the key's scopes |
...
API keys are long-lived bearer tokens prefixed with `gk_`. They are created in the admin dashboard (Developer settings) or via `POST /v1/api-keys`. The raw key is returned exactly once at creation; the database stores only a SHA-256 hash. Keys carry scopes and optional brand/event bindings:

```bash
curl -X GET http://localhost:4000/v1/events \
  -H "Authorization: Bearer gk_live_01HN..."
```
...
| `cursor` | string | none | opaque ULID |
...
| `429` | `RATE_LIMITED` | Rate limit exceeded (100 req / min / IP by default) |
...
File-type questions are rejected until upload storage/validation is implemented. Consent fields require `consentText` and a new `consentVersion` when text changes; historical consent snapshots are immutable.
