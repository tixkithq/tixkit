...
## Readiness and event launch

- `GET /organizations/{organizationId}/readiness?brandId=...` returns tenant-, organization-, and brand-scoped workspace readiness.
- `GET /events/{eventId}/launch-readiness` returns authoritative event launch blockers and warnings.
- `POST|DELETE /events/{eventId}/readiness-acknowledgements/{stepId}` records or removes an explicit human review acknowledgement.
- `POST /events/{eventId}/publish` returns HTTP 409 with code `launch_readiness_failed` and current blockers when required checks do not pass.
- `POST /events/{eventId}/duplicate` transactionally copies only explicitly selected safe configuration into a new draft.

Event `PATCH` requests include `expectedVersion`; a concurrent change returns HTTP 409 `stale_event_version`. Event cover and social images must be clean, completed `event_cover` or `event_seo_image` artifacts scoped to the same tenant, organization, brand, and event.

Authenticated test checkout sends `X-Tixkit-Test-Order: 1`. It is accepted only in capture/mock or explicit provider-test mode. Test orders are tagged and excluded from production reporting and exports.
...
| `PATCH` | `/v1/content-documents/:documentId`                             | Rename a scoped content document.                                                                                                                          |
...
| `POST`  | `/v1/content-documents/:documentId/preview`                     | Render an admin preview through the shared renderer. Event-page previews use canonical Puck data and do not mint checkout edit tokens.                     |
...
| Method | Path                                      | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ------ | ----------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GET`  | `/v1/public/events/:eventId/content-page` | Fetch allowlisted published event-page content for a published, publicly readable event. Optional `locale`; response excludes tenant, organization, brand, draft, variable, and creator metadata. The `page` payload exposes canonical Puck `puckData`, `provider`, `settings`, `discovery`, and validation metadata for first-party rendering. Public routes never return draft, archived, private, unpublished content, rendered HTML, headless blocks, render models, or raw `contentJson`. |
...
### Event Page Puck Runtime

Event pages use canonical Puck `EventPageDocumentV2` JSON as the source of truth. The document stores `editor.provider`, `editor.data`, and `settings`; public checkout consumes `PublicEventPagePayloadV2.page.puckData` through the shared Puck config/render wrapper. Checkout owns commerce chrome such as header, live tickets, resale, CTA, and footer. `@react-email/editor` and TipTap remain email-editor concerns only and are not event-page runtime fallbacks. See `docs/puck-event-page-contract.md` for the contract.
...
```json
{
  ...
  "buyerFields": {/* answers to buyer-scoped questions */},
  "items[].attendeeFields": [/* per-attendee answers */]
}
```
...
```json
{
  ...
  "status": "open",
  "currency": "USD",
  "quote": {/* server-computed totals and line breakdown */},
...
| `PATCH` | `/v1/organizations/:organizationId/members/:memberId`                                         | `settings.write` | Update a member role and replace brand/event grants              |
...
| `GET`  | `/v1/public/events/:eventId/page`           | none | Published event page payload with `page.puckData`, `provider`, `settings`, `discovery`, and validation metadata for the shared Puck renderer. Verified custom-domain slug variants resolve via `by-slug/:slug/page?host=...`. Only published, publicly readable events are exposed. |
...
| `GET`   | `/v1/events/:eventId/check-in-lists/:checkInListId/activity`        | `checkins.read`   | Read durable scan activity with summary and forward cursor                                                                 |
| `GET`   | `/v1/events/:eventId/check-in-lists/:checkInListId/activity/stream` | `checkins.read`   | Stream at-least-once check-in activity over SSE; resume with `Last-Event-ID` and de-duplicate scan IDs                     |
...
`GET /health` returns `{ "status": "ok", "timestamp": "<iso8601>" }` with no authentication and is the process liveness check. `GET /ready` performs a bounded database probe, returning `200` with `{ "status": "ready" }` or `503` with `{ "status": "not_ready" }`; use `/ready` for API load-balancer and Kubernetes readiness checks.
