export const MIGRATION_ENTITY_DEPENDENCY_ORDER = [
  'organization', 'brand', 'venue', 'event', 'occurrence', 'inventory-pool',
  'ticket-type', 'product', 'question', 'discount', 'access-code', 'buyer',
  'attendee', 'historical-order', 'ticket', 'historical-payment',
  'historical-refund', 'check-in',
] as const;
...
export interface MigrationAdapter<Configuration = unknown, Cursor = string> {
  ...
  normalize(row: ExtractedMigrationRow, context: AdapterContext): Promise<NormalizedMigrationEntity>;
  validate(entity: NormalizedMigrationEntity, context: AdapterContext): Promise<readonly MigrationIssue[]>;
}
...
export function sortEntitiesByDependency(
  ...
  return [...entities].sort((left, right) => {
    const rank = (dependencyRank.get(left.entityType) ?? Number.MAX_SAFE_INTEGER) -
      (dependencyRank.get(right.entityType) ?? Number.MAX_SAFE_INTEGER);
    return rank || left.externalId.localeCompare(right.externalId) ||
      left.sourcePosition.localeCompare(right.sourcePosition);
  });
}

export function migrationIdempotencyKey(input: {
  tenantId: string; sourceSystem: string; entityType: MigrationEntityType; externalId: string;
}): string {
  return [input.tenantId, input.sourceSystem, input.entityType, input.externalId]
    .map((part) => encodeURIComponent(part.trim().normalize('NFC'))).join(':');
}

export function chunkMigrationEntities(
  entities: readonly NormalizedMigrationEntity[], chunkSize: number,
): NormalizedMigrationEntity[][] {
  if (!Number.isSafeInteger(chunkSize) || chunkSize < 1) throw new RangeError('chunkSize must be a positive safe integer');
...
export function createHistoricalFinancialSnapshot(input: Omit<FinancialSnapshot, 'sideEffects'>): FinancialSnapshot {
  ...
  if (!/^[A-Z]{3}$/u.test(input.currency)) throw new TypeError('currency must be an uppercase ISO 4217 code');
  if (!Number.isFinite(Date.parse(input.occurredAt)) || !Number.isFinite(Date.parse(input.provenance.importedAt)))
...
export function assertHistoricalFinancialEntity(entity: NormalizedMigrationEntity): void {
  const financial = entity.entityType === 'historical-payment' || entity.entityType === 'historical-refund';
  if (!financial) {
    if (entity.financialSnapshot) throw new TypeError('financialSnapshot is only valid for historical financial entities');
    return;
  }
  if (!entity.financialSnapshot || entity.financialSnapshot.kind !== entity.entityType || entity.financialSnapshot.sideEffects !== 'suppressed')
    throw new TypeError('historical financial entities require a matching side-effect-suppressed snapshot');
}
...
export function buildDryRunReport(input: {
  ...
  for (const row of input.rows) {
    ...
    for (const issue of row.issues) { severityCounts[issue.severity] += 1; issues.push(issue); }
  }
  const bytesPerRow = input.bytesPerRow ?? 2048;
  const millisecondsPerRow = input.millisecondsPerRow ?? 3;
  if (!Number.isFinite(bytesPerRow) || bytesPerRow < 0 || !Number.isFinite(millisecondsPerRow) || millisecondsPerRow < 0)
    throw new RangeError('estimate rates must be finite and non-negative');
  return {
    mode: 'dry-run', domainWrites: 0, counts, severityCounts, rows: [...input.rows], issues,
    ...
    mappingFailures: issues.filter((issue) => issue.code.startsWith('MAPPING_')).map((issue) => issue.externalId ?? issue.sourcePosition ?? issue.code),
    ...
    estimate: { runtimeMs: Math.ceil(input.rows.length * millisecondsPerRow), storageBytes: Math.ceil(input.rows.length * bytesPerRow) },
  };
}

export type ImportedEntityActivity = {
  newSales: number; scans: number; transfers: number; edits: number;
  providerEvents: number; downstreamReferences: number;
};

export type RollbackAssessment =
  | { eligible: true; mode: 'cancel-before-commit' | 'delete-untouched-before-activation'; entityIds: readonly string[] }
...
export function assessRollback(input: {
  ...
  for (const entity of input.createdEntities) {
    if (!entity.activity) { reasons.push(`${entity.id}: activity state unavailable`); continue; }
    for (const field of ['newSales', 'scans', 'transfers', 'edits', 'providerEvents', 'downstreamReferences'] as const) {
      const count = entity.activity[field];
      if (count === undefined) reasons.push(`${entity.id}: ${field} not checked`);
      else if (!Number.isSafeInteger(count) || count < 0) reasons.push(`${entity.id}: invalid ${field} count`);
      else if (count > 0) reasons.push(`${entity.id}: has ${field}`);
    }
  }
  return reasons.length > 0
    ? { eligible: false, mode: 'corrective-plan', reasons }
    : { eligible: true, mode: 'delete-untouched-before-activation', entityIds: input.createdEntities.map(({ id }) => id) };
}

export function canCommitDryRun(report: DryRunReport): boolean {
  return report.severityCounts.fatal === 0 && report.severityCounts.error === 0 && report.counts.conflict === 0;
}
