...
describe("migration source origin policy", () => {
  it("fails closed for local, credential-bearing, insecure, and nonstandard-port origins", async () => {
    await expect(
      assertSafeMigrationOrigin("http://example.com"),
    ).rejects.toThrow("MIGRATION_SOURCE_ORIGIN_REJECTED");
    await expect(
      assertSafeMigrationOrigin("https://user:pass@example.com"),
    ).rejects.toThrow("MIGRATION_SOURCE_ORIGIN_REJECTED");
    await expect(
      assertSafeMigrationOrigin("https://localhost"),
    ).rejects.toThrow("MIGRATION_SOURCE_ORIGIN_REJECTED");
    await expect(
      assertSafeMigrationOrigin("https://127.0.0.1"),
    ).rejects.toThrow("MIGRATION_SOURCE_ORIGIN_REJECTED");
    await expect(
      assertSafeMigrationOrigin("https://example.com:8443"),
    ).rejects.toThrow("MIGRATION_SOURCE_ORIGIN_REJECTED");
  });
  ...
  it("uses source-specific authorization without accepting header injection", () => {
    ...
    expect(migrationAuthorizationHeader("eventbrite", "token")).toBe(
      "Bearer token",
    );
    expect(migrationAuthorizationHeader("hi-events", "token")).toBe(
      "Bearer token",
    );
    expect(migrationAuthorizationHeader("ticket-tailor", "token")).toBe(
      "Basic dG9rZW46",
    );
    expect(() =>
      migrationAuthorizationHeader("pretix", "token\r\nInjected: yes"),
    ).toThrow();
  });
  ...
  it("stores authenticated encrypted cursor state and rejects tampering", () => {
    ...
    expect(decryptMigrationPreparationCursor(encrypted, keyring, scope)).toBe(
      raw,
    );
    expect(() =>
      decryptMigrationPreparationCursor(`${encrypted}x`, keyring, scope),
    ...
    expect(decryptMigrationPreparationCursor(rotated, keyring, scope)).toBe(
      raw,
    );
  });
  ...
  it("loads an active write key and retains previous read keys across restart rotation", () => {
    ...
    const oldCursor = encryptMigrationPreparationCursor(
      "opaque",
      beforeRestart,
      scope,
    );
    ...
    expect(
      decryptMigrationPreparationCursor(oldCursor, afterRestart, scope),
    ).toBe("opaque");
    expect(
      encryptMigrationPreparationCursor("next", afterRestart, scope),
    ).toMatch(/^v2\.2026-07\./);
    expect(() =>
      decryptMigrationPreparationCursor(
        oldCursor,
...
describe("migration official API resource plans", () => {
  ...
  it.each([
    ...
    async ({ source, fixture, adapter }) => {
      ...
      const resourceKey =
        source.sourceSystem === "pretix" ? "resource" : "collection";
  ...
  it("transforms official Eventbrite expanded orders into order, attendee, and ticket rows", async () => {
    ...
    const ticket = await eventbriteAdapter.normalize(
      extracted.rows[2]!,
      context,
    );
  ...
  it("accepts exact Ticket Tailor list envelopes and preserves official cursor coordinates", async () => {
    ...
    const discovery = await ticketTailorAdapter.discover(
      configuration,
      context,
    );
