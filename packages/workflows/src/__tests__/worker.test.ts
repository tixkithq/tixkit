...
const schedulerConnectionConnect = vi.fn(async () => ({
  close: schedulerConnectionClose,
}));
...
const startMigrationProgressAgeRefresh = vi.fn(() => vi.fn());
...
vi.mock("@tixkit/db", () => ({
  ...
  ImportRepository: class ImportRepository {
...
describe("ensureHoldExpirationScheduler", () => {
  ...
  it("treats an already-started hold-expiration workflow as ready and closes the connection", async () => {
    ...
    await expect(ensureHoldExpirationScheduler()).resolves.toBe(
      "already_started",
    );
  ...
  it("surfaces scheduler start failures and closes the connection", async () => {
    ...
    await expect(ensureHoldExpirationScheduler()).rejects.toThrow(
...
describe("runWorker", () => {
  ...
  it("emits readiness only after all recovery schedulers are ensured", async () => {
    ...
    log.mockImplementation((message) => {
      if (String(message).startsWith("TIXKIT_WORKER_READY"))
        operations.push("ready-log");
    });
    ...
    await expect(
      Promise.race([started, Promise.resolve("running")]),
