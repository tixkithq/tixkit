...
const { migrationCommitWorkflow, migrationRollbackWorkflow } = await import(
  "../workflows/migration.js"
);
...
function installDefaults() {
  ...
  state.activities = {
    ...
    processMigrationStageActivity: vi.fn(
...
describe("migration commit workflow", () => {
  it("processes every domain stage in the required dependency order", async () => {
    ...
    expect(
      state.activities.completeMigrationCommitActivity,
  ...
  it("cancels durably between chunks and never advances to a dependent stage", async () => {
    ...
    expect(
      state.activities.cancelMigrationCommitActivity,
    ).toHaveBeenCalledOnce();
    expect(
      state.activities.completeMigrationCommitActivity,
  ...
  it("fails closed on destructive rollback when live activity makes it ineligible", async () => {
    ...
    expect(
      state.activities.executeMigrationRollbackActivity,
    ).not.toHaveBeenCalled();
    expect(
      state.activities.completeMigrationCommitActivity,
    ).not.toHaveBeenCalled();
