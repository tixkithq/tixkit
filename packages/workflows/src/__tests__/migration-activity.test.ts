...
describe('migration activities', () => {
  ...
  it('rejects incomplete entities and unsafe historical side effects before database writes', async () => {
    const committer = createProductionMigrationCommitters({} as Database).get('historical-payment');
    await expect(
      committer?.commit({
        tenantId: 'tenant_1',
    ...
    ).rejects.toThrow('MIGRATION_FINANCIAL_SNAPSHOT_REQUIRED:historical-payment');
