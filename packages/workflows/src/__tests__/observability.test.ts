...
describe("worker observability", () => {
  it("reports durable wall-clock migration stall age and resets only after persisted progress", async () => {
    ...
    output = await metrics.registry.metrics();
    expect(output).toMatch(
      /tixkit_migration_progress_age_seconds\{[^}]*phase="prepare"[^}]*\} 5/,
    );
  });
  ...
  it("exports serialized workflow spans through the OTel 2.x exporter shape", async () => {
    ...
    expect(exported[0]?.[0]?.instrumentationScope.name).toBe(
      "@temporalio/interceptor-workflow",
    );
    expect(exported[0]?.[0]?.spanContext().traceState?.serialize()).toBe(
      "vendor=value",
    );
  });
  ...
  it("does not overlap metrics pushes when the gateway never settles", async () => {
    ...
    pushMetricsToGatewayMock.mockImplementationOnce(
    ...
    try {
      ...
      await vi.waitFor(() =>
        expect(pushMetricsToGatewayMock).toHaveBeenCalledTimes(1),
      );
    } finally {
