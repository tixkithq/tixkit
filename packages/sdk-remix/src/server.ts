/**
 * Server-only entrypoint for @tixkit/remix.
 *
 * This module uses `node:crypto` and must never be imported from client-side
 * Remix code. Use the `./server` subpath import:
 *
 * ```ts
 * import { createTixkitClient, verifyTixkitWebhook } from '@tixkit/remix/server';
 * ```
 */
...
export type TixkitRemixClientConfig = {
  apiKey: string;
  apiBaseUrl?: string;
  apiVersion?: string;
  timeout?: number;
...
export function createTixkitClient(config: TixkitRemixClientConfig) {
...
export type RemixActionResult =
  | { success: true; session: Awaited<ReturnType<TixkitClient['checkout']['create']>> }
  | { success: false; status: 400 | 502; error: string; fieldErrors?: Record<string, string> };

export function createCheckoutRemixAction(
  client: TixkitClient,
  defaults: { successUrl?: string; cancelUrl?: string } = {},
) {
  return async function checkoutAction({ request }: RemixActionArgs): Promise<RemixActionResult> {
    const formData = await request.formData();
    // Reuse the same parsing logic as createCheckoutFormAction
    const eventId = optionalString(formData.get('eventId'));
    const idempotencyKey = optionalString(formData.get('idempotencyKey'));
    const { items, fieldErrors } = parseCheckoutItems(formData);
    ...
    if (Object.keys(fieldErrors).length > 0) {
      return { success: false, status: 400, error: 'Checkout form is invalid.', fieldErrors };
    }
