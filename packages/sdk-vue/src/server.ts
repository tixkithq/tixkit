/**
 * Server-only entrypoint for @tixkit/vue.
 *
 * This module uses `node:crypto` and must never be imported from client-side
 * Vue/Nuxt code. Use the `./server` subpath import:
 *
 * ```ts
 * import { createTixkitClient, verifyTixkitWebhook } from '@tixkit/vue/server';
 * ```
 */
...
export type TixkitVueClientConfig = {
  apiKey: string;
  apiBaseUrl?: string;
  apiVersion?: string;
  timeout?: number;
...
export function createTixkitClient(config: TixkitVueClientConfig) {
