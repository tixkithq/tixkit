/**
 * Vitest globalSetup for API integration tests.
 *
 * When the selected integration database URL is present, this ensures the
 * schema is migrated before DB-backed suites execute. Set
 * DB_INTEGRATION_DRIVER=mysql to target DATABASE_URL_MYSQL; otherwise the
 * default path targets DATABASE_URL/Postgres.
 *
 * This removes the tribal-knowledge requirement of running `bun run
 * db:migrate` before env-backed integration tests.
 */
export async function setup(): Promise<void> {
  const driver = process.env.DB_INTEGRATION_DRIVER === 'mysql' ? 'mysql' : 'postgres';
  const databaseUrl =
    driver === 'mysql' ? process.env.DATABASE_URL_MYSQL : process.env.DATABASE_URL;

  if (!databaseUrl) {
    // No DB configured; DB-backed suites will skip. Nothing to do.
    return;
  }

  // Import dynamically so the db package is only loaded when needed and so
  // the root config (which may be used without a DB) does not fail resolving
  // the workspace dependency.
  const { runMigrations } = await import('@tixkit/db/migrate');
  const previousDriver = process.env.DB_DRIVER;
  process.env.DB_DRIVER = driver;

  try {
    await runMigrations(databaseUrl);
  } catch (err) {
    const detail = err instanceof Error ? err.message : String(err);
    throw new Error(
      [
        'API integration test migration bootstrap failed.',
        `${driver === 'mysql' ? 'DATABASE_URL_MYSQL' : 'DATABASE_URL'} is set but migrations could not be applied.`,
        'Run `bun run db:reset` to drop and recreate the schema, then retry.',
        '',
        `Original error: ${detail}`,
      ].join('\n'),
      { cause: err },
    );
  } finally {
    if (previousDriver === undefined) {
      delete process.env.DB_DRIVER;
    } else {
      process.env.DB_DRIVER = previousDriver;
    }
  }
}

export async function teardown(): Promise<void> {
  // Nothing to clean up; test suites own their data lifecycle.
}
