...
describeWithIntegrationDatabase(
  `export route authorization DB parity (${integrationDatabaseDriver()})`,
  () => {
    ...
    beforeAll(async () => {
      ...
      eventOtherOrganization = await createEvent(
        tenantA,
        organizationOther,
        brandOther,
        'other-org',
      ...
      app.decorate('context', {
        db,
        temporalClient: { startExport, waitForExport },
      ...
      async (type) => {
        ...
        for (const exportType of EXPORT_TYPES) {
          const status = await app.inject({
            method: 'GET',
            url: `/exports/${exportType.exportId}`,
          });
          ...
          expect(download.statusCode, download.body).toBe(
            downloadContract.authorizedControl.status,
          );
    ...
    it('requires reports.read and every stored export type permission before all reads', async () => {
      const cases = [
        ...
        ...EXPORT_TYPES.map((exportType) => ({
          exportId: exportType.exportId,
          principal: principal({
            scopes: [...ALL_PERMISSIONS].filter(
              (permission) => permission !== exportType.permission,
            ),
          }),
        })),
      ];
    ...
    it('loads export authorization before any SSE subscription or download signing effect', () => {
      const source = readFileSync(
        resolve(import.meta.dirname, '../../routes/modules/reporting.ts'),
        'utf8',
      );
      ...
      const downloadScopeLoad = source.indexOf(
        'const exportJob = await loadScopedExportJob',
        downloadRoute,
      );
