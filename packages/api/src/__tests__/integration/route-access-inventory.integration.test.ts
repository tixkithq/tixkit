...
describe('API route access inventory (C-123)', () => {
  it('binds the immutable denial matrices into schema v4 evidence', async () => {
    ...
    expect(EVENT_ROUTE_AUTHORIZATION_DENIAL_CONTRACTS).toHaveLength(36);
    ...
    expect(ROUTE_AUTHORIZATION_DENIAL_CONTRACTS).toHaveLength(109);
    ...
    expect(coveredRoutes).toHaveLength(109);
    expect(coveredRoutes.flatMap((route) => route.negativeAuthorizationEvidence)).toHaveLength(453);
    expect(
      inventory.routes
    ...
    expect(policyConditions).toHaveLength(70);
    expect(
