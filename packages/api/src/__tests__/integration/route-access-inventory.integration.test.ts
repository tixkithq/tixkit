...
describe('API route access inventory (C-123)', () => {
  it('binds the immutable denial matrices into schema v4 evidence', async () => {
    ...
    expect(EVENT_ROUTE_AUTHORIZATION_DENIAL_CONTRACTS).toHaveLength(26);
    ...
    expect(ROUTE_AUTHORIZATION_DENIAL_CONTRACTS).toHaveLength(94);
    ...
    expect(coveredRoutes).toHaveLength(94);
    expect(coveredRoutes.flatMap((route) => route.negativeAuthorizationEvidence)).toHaveLength(378);
    expect(
      inventory.routes
