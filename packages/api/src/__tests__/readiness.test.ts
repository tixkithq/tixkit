...
describe("readiness service invariants", () => {
  ...
  it.each([
    ...
    "emits the authoritative ticket reason %#",
    (tickets, reasonCode) => {
      expect(
        evaluateSellableTickets(tickets, new Date("2026-01-01")),
  ...
  it("requires published document and version status and supports brand confirmation fallback", () => {
    ...
    expect(
      evaluateLifecycleContent(
        [{ ...base, eventId: null, status: "published" }],
        "evt_1",
      ),
    ).toEqual({ publicContent: false, confirmationContent: true });
    expect(
      evaluateLifecycleContent(
        [{ ...base, eventId: "evt_1", status: "archived" }],
        "evt_1",
      ),
