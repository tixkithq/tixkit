...
const workspaceReadinessQuerySchema = z
...
export const readinessRoutes: FastifyPluginAsync = async (app) => {
  ...
  const service =
    app.context.readinessServiceFactory?.(db) ??
    new ReadinessService(db, resolvePaymentMode());
    ...
    request.log.info(
      {
        ...
        incompleteStepIds: readiness.steps
          .filter(
            (step) => step.status === "incomplete" || step.status === "blocked",
          )
          .map((step) => step.id),
    ...
    request.log.info(
      {
        ...
        blockerCodes: readiness.requiredBlockers.flatMap(
          (step) => step.reasonCodes,
        ),
        durationMs: performance.now() - startedAt,
      },
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      const { eventId, stepId } = acknowledgementParamsSchema.parse(
        request.params,
      ) as {
        eventId: string;
      ...
      if (!isHumanAcknowledgementStep(stepId)) {
        throw new ValidationError(
          "Only human-review readiness steps can be acknowledged",
        );
      }
      const event = await new EventRepository(db).findById(eventId);
      if (!event) throw new NotFoundError("Event", eventId);
      ...
      const acknowledgement = await new EventReadinessAcknowledgementRepository(
        db,
      ).acknowledge(scope, {
        stepId,
