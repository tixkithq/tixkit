...
function parseStrings(value: string): string[] {
  ...
  if (!Array.isArray(parsed) || parsed.some((item) => typeof item !== 'string'))
    throw Object.assign(new Error('invalid agent authorization state'), { code: 'AGENT_STATE_INVALID' });
  return parsed;
}
...
async function databaseNow(db: Executor): Promise<string> {
  const result = await db.selectFrom('tenants').select(sql<Date>`current_timestamp`.as('now'))
...
implements AgentAuthorizationStateProvider, AgentActionInvoker {
  ...
  async load(input: { action: AgentAction; execution: AgentExecution }): Promise<AgentCurrentAuthorization> {
    this.assertEventPublish(input.action.kind, input.action.target.apiOperation,
      input.action.target.resourceType);
    return this.db.transaction().execute((tx) => this.loadState(tx, input.action.target.tenantId,
      input.action.target.resourceId, input.execution, false));
  }
  ...
  async invoke(input: Parameters<AgentActionInvoker['invoke']>[0]) {
    ...
    return this.db.transaction().setIsolationLevel('serializable').execute(async (tx) => {
      const executionRow = await tx.selectFrom('agent_executions').selectAll()
        .where('tenant_id', '=', input.tenantId).where('id', '=', input.executionId)
        .forUpdate().executeTakeFirst();
      const invocationTime = new Date(await databaseNow(tx)).getTime();
      if (!executionRow || executionRow.state !== 'running' ||
      ...
      const priorEffect = await tx.selectFrom('agent_action_effects').selectAll()
        .where('tenant_id', '=', input.tenantId).where('execution_id', '=', input.executionId)
        .executeTakeFirst();
      if (priorEffect) {
        if (priorEffect.action_digest !== input.actionDigest ||
          priorEffect.resource_type !== input.resourceType || priorEffect.resource_id !== input.resourceId ||
          priorEffect.operation !== input.operation || priorEffect.idempotency_key !== input.idempotencyKey ||
          ...
          throw Object.assign(new Error('agent effect binding changed'),
            { code: 'AGENT_AUTHORIZATION_CHANGED' });
        const result = JSON.parse(priorEffect.result) as { resourceId: string;
          resourceVersion: number; status: string };
        validateAgentActionResult(result);
        const effectedEvent = await tx.selectFrom('events').select(['status', 'version'])
          .where('tenant_id', '=', input.tenantId).where('id', '=', input.resourceId)
          .forUpdate().executeTakeFirst();
        if (agentSha256(result) !== priorEffect.result_sha256 || result.status !== 'published' ||
          (result.resourceVersion !== input.expectedResourceVersion &&
            result.resourceVersion !== input.expectedResourceVersion + 1) ||
          !effectedEvent || effectedEvent.status !== 'published' ||
          Number(effectedEvent.version) !== result.resourceVersion)
          throw Object.assign(new Error('agent effect result digest changed'),
            { code: 'AGENT_AUTHORIZATION_CHANGED' });
        return result;
      }
      const execution: AgentExecution = { id: executionRow.id, tenantId: executionRow.tenant_id,
        actionId: executionRow.action_id, actionDigest: executionRow.action_digest,
        agentPrincipalId: executionRow.agent_principal_id,
        sponsorPrincipalId: executionRow.sponsor_principal_id,
        delegationGrantId: executionRow.delegation_grant_id, approvalId: executionRow.approval_id,
        idempotencyKey: executionRow.idempotency_key,
        requestFingerprint: executionRow.request_fingerprint, state: 'running',
        resourceVersion: Number(executionRow.resource_version),
        policyVersion: Number(executionRow.policy_version), fenceToken: Number(executionRow.fence_token),
        createdAt: iso(executionRow.created_at), updatedAt: iso(executionRow.updated_at) };
      ...
      const denials = [
        ...
        current.delegation.sponsorPrincipalId !== input.sponsorPrincipalId && 'delegation_sponsor',
        ...
        !current.delegation.permissionSnapshot.includes('events:publish') && 'delegation_permission',
        !current.delegation.resourceScopes.includes(`event:${input.resourceId}`) && 'delegation_scope',
      ...
      if (denials.length > 0)
        throw Object.assign(new Error(`agent authorization changed before invocation: ${denials.join(',')}`),
          { code: 'AGENT_AUTHORIZATION_CHANGED' });

      const event = await tx.selectFrom('events').selectAll().where('tenant_id', '=', input.tenantId)
        .where('id', '=', input.resourceId).forUpdate().executeTakeFirstOrThrow();
      if (event.status === 'published') {
        const result = { resourceId: event.id, resourceVersion: Number(event.version),
          status: 'published' };
        await this.recordEffect(tx, input, result, new Date(invocationTime));
        return result;
      }
      if (event.status === 'archived') throw Object.assign(new Error('event is archived'),
        { code: 'EVENT_ARCHIVED' });
      const readiness = await new ReadinessService(tx as Database, resolvePaymentMode())
        .getEventLaunchReadiness({ tenantId: input.tenantId,
          organizationId: event.organization_id, brandId: event.brand_id, eventId: event.id,
          permissions: new Set(['events.write']) });
      if (!readiness.launchable) throw Object.assign(new Error('event readiness blocked'),
        { code: 'EVENT_NOT_READY' });
      const published = await new EventRepository(tx as Database)
        .publishIfVersion(event.id, input.expectedResourceVersion);
      if (!published) throw Object.assign(new Error('event version changed'),
        { code: 'RESOURCE_VERSION_CHANGED' });
      const result = { resourceId: published.id, resourceVersion: Number(published.version),
        status: 'published' };
      await this.recordEffect(tx, input, result, new Date(invocationTime));
      return result;
    });
  ...
  private async recordEffect(db: Executor, input: Parameters<AgentActionInvoker['invoke']>[0],
    result: { resourceId: string; resourceVersion: number; status: string }, now: Date): Promise<void> {
    await db.insertInto('agent_action_effects').values({ execution_id: input.executionId,
      tenant_id: input.tenantId, action_digest: input.actionDigest,
      resource_type: input.resourceType, resource_id: input.resourceId,
      operation: input.operation, idempotency_key: input.idempotencyKey,
      expected_policy_version: input.expectedPolicyVersion,
      expected_resource_version: input.expectedResourceVersion,
      effect_fence_token: input.expectedFenceToken, result: JSON.stringify(result),
      result_sha256: agentSha256(result), created_at: now }).execute();
  }

  private async loadState(db: Executor, tenantId: string, eventId: string,
    execution: AgentExecution, lock: boolean): Promise<AgentCurrentAuthorization> {
    const principalQuery = db.selectFrom('agent_principals').selectAll()
      .where('tenant_id', '=', tenantId).where('id', '=', execution.agentPrincipalId);
    const principal = await (lock ? principalQuery.forUpdate() : principalQuery).executeTakeFirstOrThrow();
    const delegationQuery = db.selectFrom('agent_delegations').selectAll()
      .where('tenant_id', '=', tenantId).where('id', '=', execution.delegationGrantId);
    const delegation = await (lock ? delegationQuery.forUpdate() : delegationQuery).executeTakeFirstOrThrow();
    const approvalQuery = db.selectFrom('agent_approvals').selectAll()
      .where('tenant_id', '=', tenantId).where('id', '=', execution.approvalId);
    const approval = await (lock ? approvalQuery.forUpdate() : approvalQuery).executeTakeFirstOrThrow();
    const permissionQuery = db.selectFrom('permission_grants').selectAll()
      .where('tenant_id', '=', tenantId).where('principal_type', '=', 'user')
      .where('principal_id', '=', execution.sponsorPrincipalId)
      .where('permission', '=', 'events.write').where('scope_type', '=', 'tenant')
      .where('scope_id', 'is', null);
    const permission = await (lock ? permissionQuery.forUpdate() : permissionQuery).executeTakeFirst();
    const policyQuery = db.selectFrom('agent_action_policies').selectAll()
      .where('tenant_id', '=', tenantId).where('action_kind', '=', 'event.publish');
    const policy = await (lock ? policyQuery.forUpdate() : policyQuery).executeTakeFirstOrThrow();
    const eventQuery = db.selectFrom('events').select(['id', 'version'])
      .where('tenant_id', '=', tenantId).where('id', '=', eventId);
    const event = await (lock ? eventQuery.forUpdate() : eventQuery).executeTakeFirstOrThrow();
    return {
      principal: { id: principal.id, tenantId: principal.tenant_id,
        kind: principal.kind as AgentPrincipal['kind'], sponsorPrincipalId: principal.sponsor_principal_id,
        ...
        state: principal.state as AgentPrincipal['state'], registeredAt: iso(principal.registered_at) },
      delegation: { id: delegation.id, tenantId: delegation.tenant_id,
        ...
        issuedAt: iso(delegation.issued_at), expiresAt: iso(delegation.expires_at),
        ...(delegation.revoked_at ? { revokedAt: iso(delegation.revoked_at) } : {}) },
      approval: { id: approval.id, tenantId: approval.tenant_id,
        actionDigest: approval.action_digest, ...(approval.plan_sha256 ? { planSha256: approval.plan_sha256 } : {}),
        ...
        policyVersion: Number(approval.policy_version), approvedAt: iso(approval.approved_at),
        expiresAt: iso(approval.expires_at),
        ...(approval.revoked_at ? { revokedAt: iso(approval.revoked_at) } : {}),
        ...(approval.consumed_at ? { consumedAt: iso(approval.consumed_at) } : {}) } as AgentApproval,
      ...
      currentResourceVersion: Number(event.version), currentPolicyVersion: Number(policy.policy_version),
      riskPolicyAllowed: Boolean(policy.risk_allowed), observedAt: await databaseNow(db),
    };
  }
