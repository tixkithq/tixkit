...
export function resolvePaymentMode(): PaymentMode {
  ...
  return process.env.PAYMENT_PROVIDER_TEST_MODE === "1"
    ? "provider_test"
    : "provider";
}
  ...
  return (
    input.tickets.some(
      (ticket) =>
        ...
        ticket.priceCents > 0 ||
        (ticket.minimumPriceCents ?? 0) > 0,
    ) ||
    input.products.some(
      (product) => product.status === "active" && product.priceCents > 0,
    )
  );
}
...
export function evaluateAcknowledgement(input: {
  ...
  const valid =
    input.acknowledgement.stepVersion ===
      humanAcknowledgementStepVersions[input.stepId] &&
    ...
    active.some(
      (ticket) =>
        (ticket.salesStartAt !== null &&
          Number.isNaN(new Date(ticket.salesStartAt).getTime())) ||
        (ticket.salesEndAt !== null &&
          Number.isNaN(new Date(ticket.salesEndAt).getTime())) ||
        (ticket.salesStartAt !== null &&
          ticket.salesEndAt !== null &&
...
export class ReadinessService {
  ...
  async getWorkspaceReadiness(input: {
    ...
    const brandConfigured =
      Boolean(brand.name.trim()) && Object.keys(theme).length > 0;
    ...
    const steps: Array<ReadinessStep<WorkspaceReadinessStepId>> = [
      ...
      actionStep(input.permissions, {
        ...
        priority: "recommended",
        reasonCodes: [
          senderReady ? "sender_identity_verified" : "sender_identity_missing",
        ],
        actionId: "configure_sender",
        requiredPermission: "messages.write",
        updatedAt: latestIso(
          senderIdentities.map((identity) => identity.updated_at),
        ),
        acknowledgedAt: null,
      }),
    ];
  ...
  async getEventLaunchReadiness(input: {
    ...
    const pricingValid =
      activeTickets.every(
        (ticket) =>
          ...
          (ticket.minimum_price_cents === null ||
            Number(ticket.minimum_price_cents) >= 0),
      ) &&
      feeRules.every(
        (rule) =>
          Number(rule.value) >= 0 &&
          (rule.type !== "percentage" || Number(rule.value) <= 10_000),
      );
    const requiresPayment = eventRequiresPayment({
      tickets: activeTickets.map((ticket) => ({
        ...
        minimumPriceCents:
          ticket.minimum_price_cents === null
            ? null
            : Number(ticket.minimum_price_cents),
      })),
    ...
    const publicContent =
      Number(legacyPageCount.count) > 0 || lifecycleContent.publicContent;
    ...
    const previewFingerprint = fingerprint({
      ...
      products: products.map(
        ({ updated_at: _updatedAt, ...product }) => product,
      ),
    ...
    const ackByStep = new Map(
      acknowledgements.map((ack) => [ack.step_id, ack]),
    );
    ...
    const checkoutAck = acknowledgementState(
      "checkout_consent",
      checkoutFingerprint,
    );
    const previewAck = acknowledgementState(
      "preview_review",
      previewFingerprint,
    );
    ...
    const steps: Array<ReadinessStep<EventLaunchReadinessStepId>> = [
      actionStep(input.permissions, {
        ...
        reasonCodes: basicsValid
          ...
          : [
              ...
              ...(event.timezone.trim()
                ? []
                : (["event_timezone_missing"] as const)),
      ...
      actionStep(input.permissions, {
        ...
        reasonCodes:
          ticketCurrencyCoherent && productCurrencyCoherent
            ? ["currency_coherent"]
            : [
                ...(ticketCurrencyCoherent
                  ? []
                  : (["ticket_currency_mismatch"] as const)),
                ...(productCurrencyCoherent
                  ? []
                  : (["product_currency_mismatch"] as const)),
              ],
        actionId: ticketCurrencyCoherent ? "manage_products" : "manage_tickets",
        requiredPermission: ticketCurrencyCoherent
          ? "events.write"
          : "tickets.write",
        updatedAt: iso(event.updated_at),
        acknowledgedAt: null,
      ...
      actionStep(input.permissions, {
        ...
        priority: "required",
        reasonCodes: [
          publicContent ? "public_content_published" : "public_content_missing",
        ],
        actionId: "edit_event_content",
        requiredPermission: "events.write",
        updatedAt: latestIso(
          contentDocuments.map((document) => document.updated_at),
        ),
        acknowledgedAt: null,
      }),
      actionStep(input.permissions, {
    ...
    if (this.paymentMode === "provider") {
      const testOrderIndex = steps.findIndex(
        (step) => step.id === "test_order",
      );
    ...
    steps.push(
      {
        ...
        actionId:
          prePublishable && input.permissions.has("events.write")
            ? "publish_event"
            : null,
        requiredPermission: "events.write",
        updatedAt: iso(event.updated_at),
      ...
      actionStep(input.permissions, {
        ...
        requiredPermission:
          event.status === "published" ? "events.read" : "events.write",
        updatedAt: iso(event.updated_at),
        acknowledgedAt: null,
      }),
    );
