...
export const MarketingIntegrationsMigration: Migration = {
  async up(db): Promise<void> {
    await db.schema
      ...
      .addColumn('consent_required', 'boolean', (col) => col.notNull().defaultTo(true))
  ...
  async down(db): Promise<void> {
    await db.schema.dropIndex('idx_marketing_integrations_brand').ifExists().execute();
    await db.schema.dropIndex('idx_marketing_integrations_event').ifExists().execute();
