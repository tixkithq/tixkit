...
export const NullableWebhookDeliveryEndpointMigration: Migration = {
  async up(db): Promise<void> {
    ...
    if (isMssql()) {
      ...
      await sql`alter table webhook_deliveries drop constraint webhook_deliveries_endpoint_fk`.execute(
        db,
      );
      await sql`alter table webhook_deliveries alter column endpoint_id varchar(32) null`.execute(
        db,
      );
      await sql`
        alter table webhook_deliveries
  ...
  async down(db): Promise<void> {
    ...
    if (isMssql()) {
      ...
      await sql`alter table webhook_deliveries alter column endpoint_id varchar(32) not null`.execute(
        db,
      );
      await sql`
        alter table webhook_deliveries
