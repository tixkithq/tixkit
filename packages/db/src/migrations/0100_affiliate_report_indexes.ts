...
async function dropIndex(
  ...
  if (process.env.DB_DRIVER === 'mssql') {
    await sql`drop index ${sql.raw(indexName)} on ${sql.table(tableName)}`.execute(db);
    return;
  }
  await db.schema.dropIndex(indexName).execute();
}

export const AffiliateReportIndexesMigration: Migration = {
  ...
  async down(db): Promise<void> {
    for (let index = INDEXES.length - 1; index >= 0; index -= 1) {
      const [tableName, indexName, columns] = INDEXES[index]!;
      await dropIndex(db, tableName, indexName, columns);
    }
  },
};
