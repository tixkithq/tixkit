...
async function createEventResourcePaginationIndexes(db: Kysely<unknown>): Promise<void> {
  ...
  await ensureIndex(db, {
    name: 'idx_inventory_pools_event_id',
    table: 'inventory_pools',
    columns: ['event_id', 'id'],
  });
}

async function restoreEventResourceIndexes(db: Kysely<unknown>): Promise<void> {
  await ensureIndex(db, {
  ...
  await ensureIndex(db, {
    name: 'idx_inventory_pools_event',
    table: 'inventory_pools',
    columns: ['event_id'],
  });
}

export const EventResourcePaginationIndexesMigration: Migration = {
  async up(db): Promise<void> {
    await createEventResourcePaginationIndexes(db);
    await dropIndex(db, 'ticket_types', 'idx_ticket_types_event', ['event_id']);
    await dropIndex(db, 'inventory_pools', 'idx_inventory_pools_event', ['event_id']);
  },

  async down(db): Promise<void> {
    await restoreEventResourceIndexes(db);
    await dropIndex(db, 'inventory_pools', 'idx_inventory_pools_event_id', ['event_id', 'id']);
    await dropIndex(db, 'ticket_types', 'idx_ticket_types_event_id', ['event_id', 'id']);
  },
};
