...
if (cases.length === 0)
  it.skip('agent memory integration (database URLs not configured)', () => {});
...
describe.sequential.each(cases)('agent memory lifecycle: $driver', ({ driver, url }) => {
  ...
  const audit = (id: string) => ({
    id,
    actorPrincipalId: 'user_sponsor',
    reasonCode: 'ORGANIZER_REQUEST',
    idempotencyKey: `agent-memory-${id}-2026`,
  });
  const provenance = () => ({
  ...
  beforeAll(async () => {
    ...
    const organization = await new OrganizationRepository(db).create({
      tenantId,
      name: `Memory Organization ${driver}`,
      slug: `memory-organization-${driver}`,
    });
    const brand = await new BrandRepository(db).create({
      tenantId,
      organizationId: organization.id,
      name: `Memory Brand ${driver}`,
      slug: `memory-brand-${driver}`,
    });
    organizationId = organization.id;
    brandId = brand.id;
    eventId = (
      await new EventRepository(db).create({
        ...
        organizationId: organization.id,
        brandId: brand.id,
        slug: `memory-event-${driver}`,
        title: `Memory Event ${driver}`,
        currency: 'USD',
    ...
    for (const suffix of ['empty', 'malformed', 'digest', 'extra_key', 'non_canonical']) {
      scopedEvents[suffix] = (
        await new EventRepository(db).create({
          ...
          organizationId: organization.id,
          brandId: brand.id,
          slug: `memory-${suffix}-${driver}`,
          title: `Memory ${suffix} ${driver}`,
          currency: 'USD',
          timezone: 'America/Chicago',
          startsAt: new Date('2027-01-01T18:00:00Z'),
        })
    ...
    await db
      ...
      .values({
        id: 'user_sponsor',
        tenant_id: tenantId,
        clerk_user_id: `clerk_memory_${driver}`,
        email: `memory-${driver}@example.test`,
        first_name: null,
        last_name: null,
        avatar_url: null,
        status: 'active',
    ...
    await db
      ...
      .values({
        ...
        capabilities: JSON.stringify(['memory.read']),
    ...
    await db
      ...
      .values({
        id: 'delegation_memory',
        tenant_id: tenantId,
        agent_principal_id: 'agent_memory',
        sponsor_principal_id: 'user_sponsor',
        capabilities: JSON.stringify(['memory.read']),
        resource_scopes: JSON.stringify([`event:${eventId}`]),
        permission_snapshot: JSON.stringify(['events:read']),
        issued_at: new Date(now.getTime() - 5_000),
  ...
  it('creates, inspects, corrects, exports, retrieves and physically deletes with digest-only audit', async () => {
    ...
    const createInput = {
      ...
      content: {
        ...
        summary: 'Prefer a warm concise tone',
        tone: 'warm' as const,
    ...
    await expect(
      memory.correct({
        ...
        retentionExpiresAt: createInput.retentionExpiresAt,
        audit: audit('correct_stale'),
      }),
    ).rejects.toThrow('AGENT_MEMORY_VERSION_CONFLICT');
    expect(
      await memory.retrieveForAgent({
        namespace: namespace(),
        agentPrincipalId: 'agent_memory',
        delegationId: 'delegation_memory',
        reasonCode: 'RECOMMENDATION_CONTEXT',
        recommendationId: 'recommendation_primary',
      }),
    ).toHaveLength(1);
    ...
    expect(events.map(({ operation }) => operation)).toEqual(
      expect.arrayContaining(['create', 'inspect', 'correct', 'retrieve', 'export', 'delete']),
    );
    expect(JSON.stringify(events)).not.toContain('direct concise');
    await expect(
      db
        .updateTable('agent_memory_events')
        .set({ reason_code: 'TAMPERED' })
        .where('entry_id', '=', 'memory_primary')
        .execute(),
  ...
  it('expires content using separate retention while preserving only digest audit', async () => {
    ...
    expect(
      await db
        .selectFrom('agent_memory_events')
        .select(['operation', 'previous_sha256', 'new_sha256'])
        .where('entry_id', '=', 'memory_expired')
        .where('operation', '=', 'expire')
        .executeTakeFirstOrThrow(),
    ).toMatchObject({ operation: 'expire', new_sha256: null });
  });
  ...
  it('binds event scopes to tenant ownership and immutable resource identity', async () => {
    ...
    await expect(memory.inspect(missing, audit('inspect_missing'))).rejects.toThrow(
      'AGENT_MEMORY_SCOPE_NOT_FOUND',
    );
    const foreignTenant = await new TenantRepository(db).create({
      name: `Foreign Memory ${driver}`,
    });
    const foreignOrganization = await new OrganizationRepository(db).create({
      tenantId: foreignTenant.id,
      name: `Foreign Organization ${driver}`,
      slug: `foreign-memory-organization-${driver}`,
    });
    const foreignBrand = await new BrandRepository(db).create({
      tenantId: foreignTenant.id,
      organizationId: foreignOrganization.id,
      name: `Foreign Brand ${driver}`,
      slug: `foreign-memory-brand-${driver}`,
    });
    const foreignEvent = await new EventRepository(db).create({
      tenantId: foreignTenant.id,
      organizationId: foreignOrganization.id,
      brandId: foreignBrand.id,
      slug: `foreign-memory-event-${driver}`,
      title: `Foreign Memory Event ${driver}`,
      currency: 'USD',
      timezone: 'America/Chicago',
      startsAt: new Date('2027-01-01T18:00:00Z'),
    });
    await expect(
      memory.inspect({ ...namespace(), scopeId: foreignEvent.id }, audit('inspect_foreign')),
    ).rejects.toThrow('AGENT_MEMORY_SCOPE_NOT_FOUND');
    const rebound = await new EventRepository(db).create({
      tenantId,
      organizationId,
      brandId,
      slug: `memory-rebound-${driver}`,
      title: `Memory Rebound ${driver}`,
      currency: 'USD',
      timezone: 'America/Chicago',
      startsAt: new Date('2027-01-01T18:00:00Z'),
    });
    ...
    await db
      ...
      .values({
        ...
        venue:
          persisted.venue === null || typeof persisted.venue === 'string'
            ? persisted.venue
            : JSON.stringify(persisted.venue),
        created_at: new Date(Date.now() + 86_400_000),
        updated_at: new Date(),
      })
      .execute();
    await expect(memory.inspect(reboundNamespace, audit('inspect_rebound'))).rejects.toThrow(
      'AGENT_MEMORY_RESOURCE_REBOUND',
    );
  });
  ...
  it('permanently replays an authorized absent-delete outcome without deleting later content', async () => {
    ...
    await memory.create({
      ...
      content: {
        ...
        summary: 'Prefer standard detail',
        verbosity: 'standard',
      },
      provenance: provenance(),
      retentionExpiresAt: new Date(Date.now() + 60_000).toISOString(),
  ...
  it('denies workspace, future, and wrong-sponsor delegation retrieval', async () => {
    ...
    const retrieve = () =>
      memory.retrieveForAgent({
        namespace: workspace,
        agentPrincipalId: 'agent_memory',
        delegationId: 'delegation_memory',
        reasonCode: 'RECOMMENDATION_CONTEXT',
        recommendationId: 'recommendation_workspace',
      });
    await expect(retrieve()).rejects.toThrow('AGENT_MEMORY_RETRIEVAL_DENIED');
    await db
      .updateTable('agent_delegations')
      .set({
        resource_scopes: JSON.stringify(['workspace:workspace']),
        issued_at: new Date(Date.now() + 60_000),
      })
      .where('id', '=', 'delegation_memory')
      .execute();
    await expect(retrieve()).rejects.toThrow('AGENT_MEMORY_RETRIEVAL_DENIED');
    await db
      .updateTable('agent_delegations')
      .set({ issued_at: new Date(Date.now() - 60_000), sponsor_principal_id: 'user_other' })
      .where('id', '=', 'delegation_memory')
      .execute();
    await expect(retrieve()).rejects.toThrow('AGENT_MEMORY_RETRIEVAL_DENIED');
    await db
  ...
  it('audits empty inspection, export, and agent retrieval with namespace result digests', async () => {
    ...
    await expect(
      memory.retrieveForAgent({
        ...
        delegationId: 'delegation_memory',
        reasonCode: 'RECOMMENDATION_CONTEXT',
        recommendationId: 'recommendation_empty',
      }),
    ).resolves.toEqual([]);
    const queryEvents = await db
      .selectFrom('agent_memory_events')
      .select(['operation', 'delegation_id', 'use_id'])
      .where('entry_id', '=', 'namespace_query')
      .where('scope_id', '=', scopedEvents.empty!)
      .execute();
    expect(queryEvents).toHaveLength(3);
    expect(queryEvents.find(({ operation }) => operation === 'retrieve')).toMatchObject({
      delegation_id: 'delegation_memory',
      use_id: 'recommendation_empty',
    });
  });
  ...
  it('rechecks the active sponsor and current permission grants for every agent retrieval', async () => {
    const memory = new AgentMemoryRepository(db);
    const empty = { ...namespace(), scopeId: scopedEvents.empty };
    const retrieve = (recommendationId: string) =>
      memory.retrieveForAgent({
        namespace: empty,
        agentPrincipalId: 'agent_memory',
        delegationId: 'delegation_memory',
    ...
    await db
      .updateTable('user_profiles')
      .set({ status: 'suspended', updated_at: new Date() })
      .where('id', '=', 'user_sponsor')
      .execute();
    await expect(retrieve('recommendation_suspended')).rejects.toThrow(
      'AGENT_MEMORY_RETRIEVAL_DENIED',
    );
    await db
      .updateTable('user_profiles')
      .set({ status: 'active', updated_at: new Date() })
    ...
    await expect(retrieve('recommendation_permission_removed')).rejects.toThrow(
      'AGENT_MEMORY_RETRIEVAL_DENIED',
    );
    const now = new Date();
    await db
  ...
  it.each([
    ...
    await db
      ...
      .where('id', '=', id)
      .execute();
    await expect(memory.inspect(scoped, audit(`inspect_${suffix}`))).rejects.toThrow(
      'AGENT_MEMORY_INTEGRITY_INVALID',
    );
  });
  ...
  it('physically deletes and expires corrupt rows without exposing their content', async () => {
    ...
    await db
      .updateTable('agent_memory_entries')
      .set({ content_sha256: 'buyer@example.test' })
      .where('id', '=', 'memory_malformed')
      .execute();
    await expect(
      memory.remove({
        namespace: { ...namespace(), scopeId: scopedEvents.malformed },
        entryId: 'memory_malformed',
        expectedVersion: 1,
        audit: audit('delete_corrupt'),
      }),
    ).resolves.toBe(true);
    await db
      .updateTable('agent_memory_entries')
    ...
    expect(
      await db
        .selectFrom('agent_memory_entries')
        .select('id')
        .where('id', 'in', ['memory_malformed', 'memory_digest'])
        .execute(),
    ).toHaveLength(0);
    const cleanupEvents = await db
      .selectFrom('agent_memory_events')
      .select(['operation', 'previous_sha256'])
      .where('operation', 'in', ['delete_corrupt', 'expire_corrupt'])
      .execute();
    expect(cleanupEvents).toHaveLength(2);
    expect(
      cleanupEvents.every(({ previous_sha256 }) => /^[a-f0-9]{64}$/u.test(previous_sha256!)),
    ).toBe(true);
