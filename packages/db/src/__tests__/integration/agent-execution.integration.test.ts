import { AGENT_PROTOCOL_VERSION, type AgentApproval, type AgentAuditRecord,
  type AgentDelegationGrant, type AgentExecution, type AgentPrincipal } from '@tixkit/agent-protocol';
...
function principal(tenantId: string): AgentPrincipal {
  return { id: 'agent_test', tenantId, kind: 'third_party', sponsorPrincipalId: 'organizer_test',
    capabilities: ['events.execute'], maximumAutonomy: 'execute_with_approval',
    protocolVersion: AGENT_PROTOCOL_VERSION, state: 'active',
    registeredAt: new Date(Date.now() - 120_000).toISOString() };
}

function delegation(tenantId: string): AgentDelegationGrant {
  return { id: 'delegation_test', tenantId, agentPrincipalId: 'agent_test',
    sponsorPrincipalId: 'organizer_test', capabilities: ['events.execute'],
    resourceScopes: ['event:event_test'], permissionSnapshot: ['events:write'],
...
function controlAudit(id: string) {
  return { id, actorPrincipalId: 'user_actor', reasonCode: 'TEST_AUTHORIZATION',
...
describe.sequential.each(driverCases)('agent execution persistence: $driver', ({ driver, url }) => {
  ...
  beforeAll(async () => {
    ...
    const now = new Date();
    await db.insertInto('user_profiles').values({ id: 'user_actor', tenant_id: tenantId,
      clerk_user_id: `clerk_agent_${driver}`, email: `agent-${driver}@example.test`,
      first_name: null, last_name: null, avatar_url: null, status: 'active', last_seen_at: null,
      created_at: now, updated_at: now }).execute();
    await db.insertInto('permission_grants').values({ id: 'pg_agent_control', tenant_id: tenantId,
      principal_type: 'user', principal_id: 'user_actor', permission: 'developers.write',
  ...
  it('atomically consumes one approval and converges concurrent idempotent reservations', async () => {
    ...
    await expect(repository.registerPrincipal(registeredPrincipal, controlAudit('register_test')))
      .resolves.toBeUndefined();
    await expect(repository.grantDelegation(grantedDelegation, controlAudit('grant_test')))
      .resolves.toBeUndefined();
    await expect(repository.registerPrincipal({ ...principal(tenantId), id: 'agent_substitution' },
      controlAudit('register_test'))).rejects.toThrow('AGENT_CONTROL_IDEMPOTENCY_CONFLICT');
    await expect(repository.registerPrincipal({ ...principal(tenantId), id: 'agent_denied' },
      { ...controlAudit('register_denied'), actorPrincipalId: 'user_missing' }))
      .rejects.toThrow('AGENT_CONTROL_ACTOR_DENIED');
    await db.updateTable('permission_grants').set({ permission: 'reports.read' })
      .where('id', '=', 'pg_agent_control').execute();
    await expect(repository.registerPrincipal({ ...principal(tenantId), id: 'agent_permission_denied' },
      controlAudit('register_permission_denied'))).rejects.toThrow('AGENT_CONTROL_ACTOR_DENIED');
    await db.updateTable('permission_grants').set({ permission: 'developers.write' })
      .where('id', '=', 'pg_agent_control').execute();
    const concurrentPrincipal = { ...principal(tenantId), id: 'agent_concurrent' };
    await expect(Promise.all(Array.from({ length: 8 }, () => repository.registerPrincipal(
      concurrentPrincipal, controlAudit('register_concurrent'),
    )))).resolves.toHaveLength(8);
    const concurrentDelegation = { ...delegation(tenantId), id: 'delegation_concurrent',
      agentPrincipalId: concurrentPrincipal.id };
    await expect(Promise.all(Array.from({ length: 8 }, () => repository.grantDelegation(
      concurrentDelegation, controlAudit('grant_concurrent'),
    )))).resolves.toHaveLength(8);
    const substitutions = await Promise.allSettled(Array.from({ length: 8 }, (_, index) =>
      repository.registerPrincipal({ ...principal(tenantId),
        id: index % 2 === 0 ? 'agent_race_a' : 'agent_race_b' },
    ...
    await expect(repository.grantDelegation({ ...delegation(tenantId), id: 'delegation_excess',
      capabilities: ['refunds.execute'] }, controlAudit('grant_excess'))).rejects.toThrow(
      'agent delegation is invalid',
    );
    expect(await db.selectFrom('agent_control_events').select('id')
      .where('tenant_id', '=', tenantId).execute()).toHaveLength(5);
  ...
  it('blocks claims after approval and delegation revocation', async () => {
    ...
    const approvalExecution = { ...execution(tenantId, 'execution_approval_revoked'),
      approvalId: revokedApproval.id, idempotencyKey: 'agent-execution-approval-revoked' };
    await repository.recordApproval(revokedApproval);
    await repository.reserveAndConsume({ execution: approvalExecution, approval: revokedApproval,
      requiredApproverPermission: 'events:write', now: new Date().toISOString(),
      audit: [audit(approvalExecution, 'prepared_approval_revoked', 'prepared'),
        audit(approvalExecution, 'authorized_approval_revoked', 'authorized')] });
    await expect(Promise.all(Array.from({ length: 8 }, () => repository.revokeApproval({ tenantId,
      approvalId: revokedApproval.id, audit: controlAudit('revoke_approval') }))))
      .resolves.toEqual(Array(8).fill(true));
    await expect(repository.claim({ tenantId, executionId: approvalExecution.id,
      workerId: 'worker_approval_revoked',
      audit: audit(approvalExecution, 'started_approval_revoked', 'started') })).resolves.toBeNull();
    ...
    const delegationExecution = { ...execution(tenantId, 'execution_delegation_revoked'),
      delegationGrantId: revokedDelegation.id, approvalId: delegationApproval.id,
      idempotencyKey: 'agent-execution-delegation-revoked' };
    await repository.recordApproval(delegationApproval);
    await repository.reserveAndConsume({ execution: delegationExecution, approval: delegationApproval,
      requiredApproverPermission: 'events:write', now: new Date().toISOString(),
      audit: [audit(delegationExecution, 'prepared_delegation_revoked', 'prepared'),
        audit(delegationExecution, 'authorized_delegation_revoked', 'authorized')] });
    await expect(Promise.all(Array.from({ length: 8 }, () => repository.revokeDelegation({ tenantId,
      delegationId: revokedDelegation.id, audit: controlAudit('revoke_delegation') }))))
      .resolves.toEqual(Array(8).fill(true));
    await expect(repository.claim({ tenantId, executionId: delegationExecution.id,
      workerId: 'worker_delegation_revoked',
      audit: audit(delegationExecution, 'started_delegation_revoked', 'started') })).resolves.toBeNull();
  });
  ...
  it('revokes principals and all delegations atomically before execution claim', async () => {
    ...
    const secondExecution = { ...execution(tenantId, 'execution_revocation'),
      approvalId: secondApproval.id, idempotencyKey: 'agent-execution-revocation' };
    await repository.recordApproval(secondApproval);
    await repository.reserveAndConsume({ execution: secondExecution, approval: secondApproval,
      requiredApproverPermission: 'events:write', now: new Date().toISOString(),
      audit: [audit(secondExecution, 'prepared_revocation', 'prepared'),
        audit(secondExecution, 'authorized_revocation', 'authorized')] });
    await expect(Promise.all(Array.from({ length: 8 }, () => repository.revokePrincipal({ tenantId,
      principalId: 'agent_test', audit: controlAudit('revoke_test') }))))
      .resolves.toEqual(Array(8).fill(true));
    await expect(repository.claim({ tenantId, executionId: secondExecution.id,
      workerId: 'worker_revoked', audit: audit(secondExecution, 'started_revoked', 'started') }))
      .resolves.toBeNull();
    expect((await repository.getPrincipal(tenantId, 'agent_test'))?.state).toBe('revoked');
    const storedDelegation = await db.selectFrom('agent_delegations').select('revoked_at')
      .where('tenant_id', '=', tenantId).where('id', '=', 'delegation_test').executeTakeFirstOrThrow();
    expect(storedDelegation.revoked_at).not.toBeNull();
    expect(await db.selectFrom('agent_audit_events').select('id')
      .where('execution_id', '=', secondExecution.id).execute()).toHaveLength(2);
    const controlEvents = await db.selectFrom('agent_control_events').selectAll()
      .where('tenant_id', '=', tenantId).execute();
    expect(controlEvents).toHaveLength(9);
    expect(controlEvents.find(({ id }) => id === 'revoke_test')).toMatchObject({ actor_principal_id: 'user_actor',
      operation: 'revoke', target_id: 'agent_test', reason_code: 'TEST_AUTHORIZATION' });
    await expect(db.updateTable('agent_control_events').set({ reason_code: 'TAMPERED' })
      .where('id', '=', 'revoke_test').execute()).rejects.toThrow(/immutable/u);
    await expect(db.deleteFrom('agent_control_events').where('id', '=', 'revoke_test').execute())
      .rejects.toThrow(/immutable/u);
  });
});
