...
export class DiscountCodeRepository extends BaseRepository {
  ...
  async reserveForCheckout(input: DiscountReservationInput): Promise<DiscountReservationResult> {
    return this.db.transaction().execute(async (trx) => {
      return new DiscountCodeRepository(trx).reserveForCheckoutInTransaction(input);
    });
  }

  async reserveForCheckoutInTransaction(
    input: DiscountReservationInput,
