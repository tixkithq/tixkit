...
const SENSITIVE_KEY =
  /(?:password|passwd|secret|token|api[_-]?key|credential|private[_-]?key|cvv|cvc|card|pan|ssn|authorization|access[_-]?code|address|date[_-]?of[_-]?birth|buyer|customer)/iu;
const CREDENTIAL_VALUE =
  /(?:sk_(?:live|test)_[A-Za-z0-9]+|AKIA[0-9A-Z]{16}|gh[pousr]_[A-Za-z0-9]{20,}|eyJ[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+|-----BEGIN [A-Z ]*PRIVATE KEY-----|(?:bearer|basic)\s+[A-Za-z0-9+/._~=-]+)/iu;
const EMAIL = /\b[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}\b/iu;
const PHONE = /(?:\+?\d[\d .()-]{8,}\d)/u;
const LONG_DIGITS = /\b\d{12,19}\b/u;
const POSTAL_ADDRESS =
  /\b\d{1,6}\s+[A-Za-z0-9 .'-]+\s(?:street|st|road|rd|avenue|ave|boulevard|blvd|lane|ln|drive|dr|court|ct)\b/iu;
...
const SOURCE_REFERENCE =
  /^(?:event|project|import|action|recommendation):[A-Za-z0-9][A-Za-z0-9_-]{1,127}$/u;
...
function inspect(value: unknown, depth = 0): void {
  ...
  if (typeof value === 'string') {
    ...
    if (
      text.length > 2_000 ||
      !/^[\x20-\x7E]*$/u.test(text) ||
      CREDENTIAL_VALUE.test(text) ||
      EMAIL.test(text) ||
      PHONE.test(text) ||
      LONG_DIGITS.test(text) ||
      POSTAL_ADDRESS.test(text) ||
      BIRTH_DATE.test(text) ||
      FULL_NAME.test(text)
    )
  ...
  if (Array.isArray(value)) {
    if (value.length > 50)
      throw new AgentProtocolValidationError('agent memory array is too large');
  ...
  if (entries.length > 50)
    throw new AgentProtocolValidationError('agent memory object is too large');
...
export function normalizeAgentMemoryContent(content: AgentMemoryContent): AgentMemoryContent {
  const allowed =
    content.kind === 'organizer_preferences'
  ...
  if (content.kind === 'organizer_preferences') {
    if (
      ...
      !['concise', 'warm', 'formal', 'direct'].includes(content.tone)
    )
      throw new AgentProtocolValidationError('agent memory tone is invalid');
    if (
      content.verbosity !== undefined &&
      !['brief', 'standard', 'detailed'].includes(content.verbosity)
    )
    ...
    if (
      ...
      !/^[A-Za-z_]+(?:\/[A-Za-z_+-]+)+$/u.test(String(result.timezone))
    )
    ...
    if (content.facts !== undefined) {
      ...
      result.facts = content.facts.map((fact) => {
        if (
          !fact ||
          !['objective', 'constraint', 'decision'].includes(fact.kind) ||
...
export function normalizeAgentMemoryProvenance(
  ...
  return {
    ...
    actorPrincipalId: provenance.actorPrincipalId,
...
export function validateAgentMemoryWrite(input: {
  ...
  if (
    ...
    (provenance.sourceReference !== undefined &&
      !SOURCE_REFERENCE.test(provenance.sourceReference)) ||
    (provenance.type === 'organizer' &&
  ...
  if (
    ...
    expires <= now ||
    expires - now > MAX_RETENTION_MS ||
    observed > now ||
    now - observed > MAX_PROVENANCE_AGE_MS
  )
