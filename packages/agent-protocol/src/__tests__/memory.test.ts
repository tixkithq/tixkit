...
const base = {
  ...
  retentionExpiresAt: '2026-10-12T12:00:00.000Z',
  ...
  it.each([
    ...
    { content: { kind: 'organizer_preferences', summary: 'Unsafe', apiToken: 'abc' } },
    {
      content: {
        ...
        summary: `Use ${['AKIA', 'IOSFODNN7EXAMPLE'].join('')}`,
      },
    },
    ...
    { provenance: { type: 'import', actorPrincipalId: 'user_sponsor', observedAt: base.now } },
    ...
    expect(() => validateAgentMemoryWrite({ ...base, ...override } as typeof base)).toThrow(
  ...
  it('normalizes equivalent Unicode before digesting', () => {
    expect(
      validateAgentMemoryWrite({
        ...base,
        content: { ...base.content, summary: 'Ｐrefer a warm tone' },
      }),
    ).toBe(
      validateAgentMemoryWrite({
        ...base,
