...
Experimental organizer-first protocol at version `2026-07-11`. The package is a public-release candidate, but publication and MIT licensing remain subject to the repository's pending legal and protected-release gates.
...
Managed Cloud releases must pin protocol version `2026-07-11` through their Cloud/core compatibility manifest. Changes to canonical bytes, action digests, authorization inputs, approval semantics, audit envelopes, schema keywords, or memory scope are protocol changes and require compatibility evidence.
