...
The root exports protocol versions and the digest-bound action registry; principal, delegation, action, immutable plan definition, mutable plan state, approval, authorization and audit contracts; canonical hashing and validation helpers; `DurableAgentExecutionService`; execution-store and invoker interfaces; and scoped memory normalization and validation contracts. `./schema` resolves to the current `2026-08-04` full-action validation overlay, which retains the `2026-07-22` wire protocol and enforces exact autonomy plus typed `report.read`, `content.prepare`, `campaign.prepare` and `event.update` payloads. Retained action schemas use explicit versioned exports through `./schemas/2026-08-04`; immutable platform schemas retain every version through `./schemas/agent-action-contracts/2026-08-04`. The registry marks `event.publish` as plan-supported and `event.read`, `report.read`, `readiness.read`, `event.prepare`, `content.prepare`, `campaign.prepare` and `event.update` as direct-only; reserved actions remain unavailable.
...
Resolve the registry-selected `prepareInputSchema`, `resolvedPayloadSchema`, or `resultSchema` JSON Pointer to validate action-specific payload and result semantics. The current action overlay enforces kind/autonomy/target structure and exact `report.read`, `content.prepare` and `event.update` payloads; the action-contract registry enforces coherent aggregate reports, bounded event-page content, typed event preparation/update inputs, mutation-ready media boundaries, previews and execution results. Other runtimes must enforce equivalent canonical-byte, depth and aggregate-coherence keywords and the same two-stage validation.
...
Managed Cloud releases must pin action protocol version `2026-07-22`, current action validation schema version `2026-08-04`, platform plan protocol version `2026-07-27`, action-registry contract version `2026-08-04`, and the registry/schema digests through their Cloud/core compatibility manifest. Changes to canonical bytes, action or plan digests, authorization inputs, approval semantics, registry policy, audit envelopes, schema keywords, or memory scope are protocol changes and require compatibility evidence. Immutable prior action schemas remain exported byte-for-byte at their versioned paths; `./schema` advances to the `2026-08-04` overlay so default consumers enforce the runtime's fail-closed autonomy, report-read, content/campaign-preparation and update-payload contracts.
...
HTTP API release `2026-08-04` retains every `2026-08-03` schema byte-for-byte and adds direct-only, aggregate-only `report.read`. It binds an exact closed range and coherent event-sales projection to one event version, exposes no row-level buyer or attendee data, fails closed on relevant drift, and creates no approval, execution, plan, export or product mutation authority.
