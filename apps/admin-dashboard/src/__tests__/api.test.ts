...
describe('AdminApi settings fixtures', () => {
  it('derives all workspace readiness steps and progresses required state to ready', async () => {
    ...
    expect(result.data.steps.map(({ id, priority }) => ({ id, priority }))).toEqual([
    ...
    if (ready.ok) {
      ...
      expect(ready.data.steps.filter((step) => step.priority === 'recommended')).toEqual(
        expect.arrayContaining([expect.objectContaining({ status: 'incomplete' })]),
      );
    }
