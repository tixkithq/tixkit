...
describe("AdminApi settings fixtures", () => {
  it("derives all workspace readiness steps and progresses required state to ready", async () => {
    const result = await adminApi.getWorkspaceReadiness("org_demo", "brd_demo");
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.data.complete).toBe(false);
    expect(
      result.data.steps.map(({ id, priority }) => ({ id, priority })),
    ).toEqual([
      { id: "workspace_selection", priority: "required" },
      { id: "brand_identity", priority: "required" },
      { id: "payment_path", priority: "required" },
      { id: "team_access", priority: "recommended" },
      { id: "legal_configuration", priority: "recommended" },
      { id: "sender_identity", priority: "recommended" },
    ]);
    expect(result.data.steps).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({ id: "brand_identity", status: "incomplete" }),
        expect.objectContaining({ id: "payment_path", status: "complete" }),
        expect.objectContaining({ id: "team_access", status: "incomplete" }),
      ]),
    );

    const activation = await adminApi.updateBrand("brd_demo", {
      theme: { primaryColor: "#222222" },
    });
    expect(activation.ok).toBe(true);
    const ready = await adminApi.getWorkspaceReadiness("org_demo", "brd_demo");
    expect(ready.ok).toBe(true);
    if (ready.ok) {
      expect(ready.data.complete).toBe(true);
      expect(
        ready.data.steps.filter((step) => step.priority === "recommended"),
      ).toEqual(
        expect.arrayContaining([
    ...
    await adminApi.updateBrand("brd_demo", { theme: {} });
  });
  ...
  it("updates brand settings and creates domains through the typed API", async () => {
    ...
    const domainResult = await adminApi.addBrandDomain(
      brand.id,
...
describe("AdminApi content document contract", () => {
  it("lists content documents through a typed page envelope", async () => {
    const result = await adminApi.listContentDocuments({
      channel: "email",
      limit: 5,
    });
    expect(result.ok).toBe(true);
...
describe("AdminApi.updateTicketType", () => {
  ...
  it("creates, lists, and deletes access rules for locked tickets", async () => {
    ...
    if (afterDelete.ok) {
      expect(
        afterDelete.data.some((rule) => rule.id === createRule.data.id),
      ).toBe(false);
    }
  });
  ...
  it("creates locked tickets with access rules and a new inventory pool atomically through the batch API", async () => {
    ...
    expect(result.data.accessRules).toEqual([
      expect.objectContaining({
        ticketTypeId: result.data.ticketType.id,
        value: "ATOMICVIP",
      }),
    ]);
  });
  ...
  it("updates tickets and appends access rules through the batch API", async () => {
    ...
    const updateResult = await adminApi.updateTicketTypeBatch(
      createResult.data.ticketType.id,
      {
        ticketType: {
  ...
  it("creates, lists, and updates products and product categories", async () => {
    const categoryResult = await adminApi.createProductCategory(
      "evt_demo_001",
      {
        name: "Merch",
        sortOrder: 3,
    ...
    const categoriesResult =
      await adminApi.listProductCategories("evt_demo_001");
...
describe("AdminApi checkout questions", () => {
  it("creates, updates, and deletes event checkout questions through the typed API", async () => {
    ...
    const updateResult = await adminApi.updateCheckoutQuestion(
    ...
    if (listResult.ok) {
      expect(
        listResult.data.some(
          (question) => question.id === createResult.data.id,
        ),
      ).toBe(true);
    }

    const deleteResult = await adminApi.deleteCheckoutQuestion(
      createResult.data.id,
    );
...
describe("AdminApi event fee policy", () => {
  it("reads and updates event fee pass-through settings", async () => {
    ...
    const update = await adminApi.updateEventFeePolicy("evt_demo_001", {
      expectedVersion: initial.ok ? initial.data.eventVersion : 1,
...
describe("AdminApi reports date range", () => {
  ...
  it("returns attendance, promo, conversion, and affiliate reports through typed methods", async () => {
    ...
    if (attendance.ok) {
      ...
      expect(attendance.data.totalAttendees).toBeGreaterThanOrEqual(
        attendance.data.checkedIn,
      );
    }
    if (promo.ok) {
      expect(promo.data.discountCodes[0]).toHaveProperty(
