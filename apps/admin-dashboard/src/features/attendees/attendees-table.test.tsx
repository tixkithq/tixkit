...
describe('Attendees table columns', () => {
  ...
  it('has name column for search', () => {
    const columns = getAttendeeColumns()
    const nameCol = columns.find((c) => (c as unknown as { accessorKey?: string }).accessorKey === 'name')
    expect(nameCol).toBeDefined()
  })

  it('has status column', () => {
    const columns = getAttendeeColumns()
    const statusCol = columns.find((c) => (c as unknown as { accessorKey?: string }).accessorKey === 'status')
    expect(statusCol).toBeDefined()
  })

  it('has checkInStatus column', () => {
    const columns = getAttendeeColumns()
    const checkInCol = columns.find((c) => (c as unknown as { accessorKey?: string }).accessorKey === 'checkInStatus')
    expect(checkInCol).toBeDefined()
  })

  it('has ticketTypeName column', () => {
    const columns = getAttendeeColumns()
    const ttCol = columns.find((c) => (c as unknown as { accessorKey?: string }).accessorKey === 'ticketTypeName')
