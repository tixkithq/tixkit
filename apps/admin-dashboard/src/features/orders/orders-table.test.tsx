...
describe('Orders table columns', () => {
  ...
  it('has buyerEmail column for search', () => {
    const columns = getOrderColumns()
    const buyerCol = columns.find((c) => (c as unknown as { accessorKey?: string }).accessorKey === 'buyerEmail')
    expect(buyerCol).toBeDefined()
  })

  it('has totalCents column', () => {
    const columns = getOrderColumns()
    const totalCol = columns.find((c) => (c as unknown as { accessorKey?: string }).accessorKey === 'totalCents')
