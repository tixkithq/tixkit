...
vi.mock('@/components/ui/select', async () => {
  ...
  function Select({ value, onValueChange, children }: { value: string; onValueChange: (v: string) => void; children: React.ReactNode }) {
    ...
    return React.createElement(SelectContext.Provider, { value: { value, onValueChange, items, registerItem } }, children)
  }

  function SelectTrigger({ className, ...props }: { className?: string; children?: React.ReactNode }) {
...
describe('RefundDialog', () => {
  ...
  it('calls refundOrder with full refund (no amountCents) and fires onSuccess', async () => {
    const spy = vi
      .spyOn(adminApi, 'refundOrder')
      .mockResolvedValue({
        ok: true,
        data: { orderId: 'ord_test_001', refundAmount: 10_000, status: 'pending', message: 'Refund workflow started' },
      })
    ...
    await waitFor(() => {
      expect(spy).toHaveBeenCalledWith('ord_test_001', expect.objectContaining({
  ...
  it('resets form state when reopened for a different order', () => {
    ...
    const updatedTextarea = screen.getByPlaceholderText(/Describe the refund reason/) as HTMLTextAreaElement
...
describe('RefundDialog partial refund mode', () => {
  ...
  it('switches to partial mode, enters amount, and submits with amountCents', async () => {
    const spy = vi
      ...
      .mockResolvedValue({
        ok: true,
        data: { orderId: 'ord_test_001', refundAmount: 5_000, status: 'pending', message: 'Refund workflow started' },
      })
    const onOpenChange = vi.fn()
    const onSuccess = vi.fn()
  ...
  it('validates partial amount cannot exceed remaining refundable balance', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/cannot exceed the remaining refundable balance/)).toBeInTheDocument()
    })
