...
function categoryLabel(product: AdminProduct, categories: AdminProductCategory[]): string {
  if (!product.categoryId) return 'Uncategorized';
  return (
    categories.find((category) => category.id === product.categoryId)?.name ?? 'Unknown category'
  );
}
  ...
  if (!payload) {
    return {
      ...
      error: {
        ...
        message: 'Name, price, and a positive per-order limit are required.',
      },
    };
  }
