...
describe('feePolicyFormSchema', () => {
  it('accepts buyer-paid percentage fee rules', () => {
    const result = feePolicyFormSchema.safeParse({
      ...
      rules: [{ name: 'Service fee', type: 'percentage', value: 500, appliedTo: 'per_ticket' }],
    });
  ...
  it('rejects percentage rules over 100%', () => {
    const result = feePolicyFormSchema.safeParse({
      passFeesToBuyer: true,
      rules: [{ name: 'Service fee', type: 'percentage', value: 10_001, appliedTo: 'per_ticket' }],
    });
  ...
  it('adds buyer-paid fees to the buyer total', () => {
    expect(
      buildFeePolicyExamples(ticketTypes, {
        passFeesToBuyer: true,
        rules: [
          { name: 'Service fee', type: 'percentage', value: 500, appliedTo: 'per_ticket' },
          { name: 'Order fee', type: 'fixed', value: 125, appliedTo: 'per_order' },
        ],
      }),
    ).toEqual([
