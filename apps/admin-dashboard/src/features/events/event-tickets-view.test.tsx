...
const adminApiMock = vi.hoisted((): EventTicketsAdminApiMock => ({
  getEvent: vi.fn(),
...
describe("EventTicketsView occurrences", () => {
  it("renders event occurrences and resolves occurrence-scoped ticket labels", async () => {
    ...
    expect(
      within(fridayTicketRow).getByText("Friday evening"),
    ).toBeInTheDocument();
    ...
    expect(
      within(weekendTicketRow).getByText("All occurrences"),
    ).toBeInTheDocument();
  });
});
...
describe("EventTicketsView waitlist claim links", () => {
  it("reports claim-link copy only after the clipboard write succeeds", async () => {
    ...
    fireEvent.click(
      within(row as HTMLTableRowElement).getByRole("button", { name: "Offer" }),
    );

    await waitFor(() => {
      expect(adminApiMock.offerWaitlistEntry).toHaveBeenCalledWith(
        "evt_1",
        waitlistEntry.id,
      );
    });
    await waitFor(() => {
      expect(
        view.getByLabelText(`Claim link for ${waitlistEntry.email}`),
      ).toHaveValue(claimUrl);
    });
  ...
  it("keeps the claim link available when offer-time clipboard access is unavailable", async () => {
    ...
    expect(toast.error).toHaveBeenCalledWith(
      "Claim link ready. Copy it manually from the row.",
    );
  });
  ...
  it("does not show copy success when the row Copy button clipboard write is rejected", async () => {
    ...
    const claimInput = await view.findByLabelText(
      `Claim link for ${waitlistEntry.email}`,
    );
...
describe("EventTicketsView resale", () => {
  it("renders resale policy controls and active listings", async () => {
    ...
    expect(
      within(panel).getByRole("switch", { name: /Resale policy/i }),
  ...
  it("saves resale policy edits and refreshes policy data", async () => {
    ...
    fireEvent.click(
      within(panel).getByRole("button", { name: /Save resale policy/i }),
    );
...
describe("EventTicketsView fee policy", () => {
  it("renders fee impact by ticket type using current ticket prices", async () => {
    ...
    expect(
      within(fridayRow as HTMLTableRowElement).getAllByText("$35.00"),
    ).toHaveLength(2);
    expect(
      within(fridayRow as HTMLTableRowElement).getByText("$1.32"),
    ).toBeInTheDocument();
    expect(
      within(fridayRow as HTMLTableRowElement).getByText("$1.75"),
    ).toBeInTheDocument();
    expect(
      within(fridayRow as HTMLTableRowElement).getByText("$38.07"),
    ).toBeInTheDocument();

    const weekendRow = within(panel).getByText("Weekend pass").closest("tr");
    expect(weekendRow).not.toBeNull();
    expect(
      within(weekendRow as HTMLTableRowElement).getAllByText("$80.00"),
    ).toHaveLength(2);
    expect(
      within(weekendRow as HTMLTableRowElement).getByText("$2.62"),
    ).toBeInTheDocument();
    expect(
      within(weekendRow as HTMLTableRowElement).getByText("$4.00"),
    ).toBeInTheDocument();
    expect(
      within(weekendRow as HTMLTableRowElement).getByText("$86.62"),
    ).toBeInTheDocument();
  });
  ...
  it("autosaves pass-through changes without a fee save button", async () => {
    ...
    const panel = await view.findByTestId("fee-policy-card");
    expect(
      within(panel).queryByRole("button", { name: /Save Fees/i }),
    ).not.toBeInTheDocument();

    fireEvent.click(
      within(panel).getByRole("switch", { name: /Pass fees to buyers/i }),
    );
    ...
    expect(
      within(fridayRow as HTMLTableRowElement).getByText("$31.93"),
    ).toBeInTheDocument();
