...
describe('questionFormSchema', () => {
  ...
  it('requires a value when a conditional source field is selected', () => {
    ...
    expect(result.success).toBe(false);
    if (!result.success) {
      expect(result.error.issues.some((issue) => issue.path.includes('conditionalValue'))).toBe(
        true,
      );
    }
  });
...
describe('conditionMatches', () => {
  ...
  it('matches not_equals for scalar, multi-value, and empty answers', () => {
    ...
    expect(conditionMatches(conditionalQuestion('not_equals'), { q_source: ['GA', 'VIP'] })).toBe(
      false,
    );
  ...
  it('matches contains against every selected value without treating an empty answer as visible', () => {
    ...
    expect(
      conditionMatches(conditionalQuestion('contains', 'IP'), { q_source: ['GA', 'VIP'] }),
    ).toBe(true);
