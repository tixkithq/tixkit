...
export default function ConfirmationClient() {
  ...
  const loadSession = useCallback(
    async (id: string, token?: string, stripeClientSecret?: string) => {
      const loaded = await checkoutApi.getSession(id, token, token ? undefined : stripeClientSecret)
      setSession(loaded)
      if (loaded.eventId) {
        try {
          ...
          <Card>
            <CardContent className='flex flex-col items-center gap-3 py-10 text-center'>
              ...
              <p className='text-sm text-muted-foreground'>
                We are confirming your payment with the processor. This page
    ...
    return (
      <div className='flex flex-col items-center gap-3 text-center'>
        ...
        <div className='space-y-1'>
          ...
          <p className='text-sm text-muted-foreground'>
            Thank you. Your tickets are on the way.
          </p>
        </div>
      </div>
    )
