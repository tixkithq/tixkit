...
export function OrderSummary({ items, tickets, currency, quote, presetTotalCents }: Props) {
  const lines = items
    .map((item) => {
  ...
  return (
    <div className="space-y-4">
      {lines.length === 0 ? (
        <p className="text-sm text-muted-foreground">Select tickets to see your order summary.</p>
      ) : (
        <ul className="space-y-3">
          {lines.map((line) => (
            <li key={line.name} className="flex items-start justify-between gap-3 text-sm">
      ...
      <dl className="space-y-2 text-sm">
        ...
        {feeCents > 0 ? <Row label="Fees" value={formatCurrency(feeCents, currency)} /> : null}
        {taxCents > 0 ? <Row label="Tax" value={formatCurrency(taxCents, currency)} /> : null}
      </dl>
