...
export function BrandFooter({ brand }: { brand: ResolvedBrand }) {
  ...
  if (brand.legalUrls.terms)
    links.push({ label: 'Terms', href: brand.legalUrls.terms })
  if (brand.legalUrls.privacy)
    links.push({ label: 'Privacy', href: brand.legalUrls.privacy })
  if (brand.legalUrls.refundPolicy)
    links.push({ label: 'Refund policy', href: brand.legalUrls.refundPolicy })
  if (brand.supportUrl)
    links.push({ label: 'Support', href: brand.supportUrl })
