...
describe('buildOperationExamples', () => {
  ...
  it.each([
  ...
  ])('renders %s callbacks with every runtime-required header', (scheme, requiredHeaders) => {
    const examples = buildOperationExamples({
      method: 'POST',
    ...
    expect(examples.typescript).toContain('providerCallbackReceiver.verify(request)');
  ...
  it('does not invent authentication for an anonymous operation', () => {
    ...
    expect(examples.typescript).toContain("anonymousClient.request('GET', path)");
  });
});
