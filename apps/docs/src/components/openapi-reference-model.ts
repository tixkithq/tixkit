...
function securityRequirements(value: unknown): readonly Record<string, readonly string[]>[] {
    ...
    return Object.fromEntries(
      Object.entries(requirement).map(([scheme, scopes]) => {
        if (!Array.isArray(scopes) || scopes.some((scope) => typeof scope !== 'string')) {
          throw new Error(`OpenAPI security scopes for ${scheme} must be strings.`);
        }
...
export function normalizeOperations(document: OpenApiDocument, expectedVersion: string) {
  ...
  for (const [path, pathValue] of Object.entries(document.paths)) {
    if (!isRecord(pathValue)) throw new Error(`OpenAPI path ${path} must be an object.`);
    ...
    for (const [method, operationValue] of Object.entries(pathValue)) {
      ...
      if (!isRecord(operationValue) || typeof operationValue.operationId !== 'string') {
        throw new Error(`OpenAPI ${method.toUpperCase()} ${path} requires an operationId.`);
      }
      ...
      operations.push({
        ...
        description:
          typeof operationValue.description === 'string' ? operationValue.description : '',
        security:
        ...
        responses: isRecord(operationValue.responses) ? operationValue.responses : {},
      });
    }
  }
