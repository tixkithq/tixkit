...
const sdkSpec = requireExactPackageSpec(
  options["sdk-spec"],
  "@tixkit/js",
  "sdk-spec",
);
...
try {
  const get = async (name) => {
    const response = await fetch(
      new URL(name, `${base.toString().replace(/\/$/u, "")}/`),
    );
    if (!response.ok)
  ...
  const trustedDirectory = resolve(
    options["trusted-contracts-root"],
    manifest.apiVersion,
  );
  ...
  for (const name of artifactNames) {
    const bytes =
      name === "release-manifest.json" ? manifestBytes : await get(name);
  ...
  const packExactRegistryPackage = (spec, expectedName) => {
    const metadata = JSON.parse(
      execFileSync(
        "npm",
        ["view", spec, "name", "version", "dist.integrity", "--json"],
        {
          encoding: "utf8",
        },
      ),
    ...
    const packed = JSON.parse(
      execFileSync(
        "npm",
        ["pack", spec, "--json", "--pack-destination", temp],
        {
          encoding: "utf8",
        },
      ),
  ...
  const contractTarball = packExactRegistryPackage(
    contractTestsSpec,
    "@tixkit/contract-tests",
  );
...
`,
  ...
  console.log(
    `Verified public API ${manifest.apiVersion}, ${sdkSpec}, and ${contractTestsSpec}.`,
  );
} finally {
