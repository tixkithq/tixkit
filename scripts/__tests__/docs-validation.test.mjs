...
test("frontmatter parser and validator accept a complete public page", () => {
  ...
  assert.deepEqual(
    validateFrontmatter(parsed.data, "docs/public/test.mdx"),
    [],
  );
});
...
test("frontmatter checks fail on missing files and seeded invalid metadata", () => {
  withRoot((root) => {
    assert.deepEqual(checkFrontmatter(root), [
      "docs/public: no public documentation pages found",
    ]);
    write(
      root,
    ...
    assert.ok(
      errors.some((error) => error.includes("invalid audience unknown")),
    );
    assert.ok(
      errors.some((error) =>
        error.includes("public content cannot use status internal"),
      ),
    );
  });
});
...
test("navigation validation rejects missing targets, duplicates, and orphan pages", () => {
  ...
  assert.ok(
    errors.some((error) => error.includes("duplicate routeId apiReference")),
  );
  assert.ok(
    errors.some((error) => error.includes("duplicate sibling label Endpoints")),
  );
  assert.ok(
    errors.some((error) =>
      error.includes("targets missing page /reference/api"),
    ),
  );
...
test("package README validation rejects missing and boilerplate documentation", () => {
  withRoot((root) => {
    ...
    assert.ok(
      errors.some((error) =>
        error.includes("missing required heading Purpose"),
      ),
    );
...
test("SDK guide sections stay synchronized in manifest and local search", () => {
  ...
  const search = JSON.parse(
    readFileSync("apps/docs/public/search-index.json", "utf8"),
  );
...
test("documentation workflows trigger for executable SDK demo changes", () => {
  for (const path of [
    ...
    ".github/workflows/trusted-docs-ci.yml",
  ]) {
