...
if [ "${DB_DRIVER}" = postgres ]; then
  ...
  : "${DR_DATABASE_VERIFY_COMMAND:?DR_DATABASE_VERIFY_COMMAND is required}"
  DATABASE_URL_MYSQL="${DATABASE_URL_MYSQL}" MYSQL_BACKUP_FILE="${db_artifacts[0]}" DR_ISOLATED_TARGET=1 \
    DR_VERIFY_COMMAND="${DR_DATABASE_VERIFY_COMMAND}" DR_RESTORE_TARGET_ID="${DR_DATABASE_TARGET_ID}" \
...
OBJECT_STORAGE_BACKUP_FILE="${object_artifacts[0]}" DR_ISOLATED_TARGET=1 \
...
"${DR_TEMPORAL_RESTORE_COMMAND}"
export DR_TEMPORAL_EVIDENCE_FILE="${DR_TEMPORAL_RESTORE_EVIDENCE}"
"${DR_TEMPORAL_EVIDENCE_VERIFY_COMMAND}"
